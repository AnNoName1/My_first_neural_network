...
public:
	...
	double ForwardFeed(string param) {
		...
		double prediction = 0;
		for (int i = 0; i < size[layers - 1]; i++) {
			cout << i << " : " << neurons[layers - 1][i].value << endl;
			if (neurons[layers - 1][i].value > max) {
				max = neurons[layers - 1][i].value;
				prediction = i;
			}
...
int main()
{
    ...
    if (to_study) 
    {
        ...
        for (int e = 0; ra / n * 100 < 100; e++) 
        {
            ...
            for (int i = 0; i < n; i++) 
            {
                ...
                if (result == rresult) 
                {
                    //cout << "Network guessed right " << rresult << "\t\t\t****" << endl;
                    ra++;
                }
                else {
                 //   cout << "Network made a mistake " << result<<endl;
            ...
            cout << "Time it took: " << time / 1000 << " ms \t\t\tEpoch time:" << epoch_stop - epoch_start << endl;
            time = 0;
            if (ra > maxra) 
            {
                maxra = ra;
	...
	if (to_start_test) {
		...
		cout << "this is supposed to be number " << result  << "\n\n";
		cout << "Is that true?...";
		cin >> right_res;
		if (right_res != result ) {
			cout << "correcting my mistake";
			nn.BackPropogation(result, right_res, 0.15);
			nn.SaveWeights();
		}
	}
    fin.close();
